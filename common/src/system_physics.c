...
{
    struct C_Physics_2d* physics = (struct C_Physics_2d*) ecdb->_componentArrays[physics_2d_handle];
    struct C_Input* inputs = (struct C_Input*) ecdb->_componentArrays[inputs_handle];
...
{
    struct C_Transform* transforms = (struct C_Transform*) ecdb->_componentArrays[transforms_handle];
    struct C_Physics_2d* physics = (struct C_Physics_2d*) ecdb->_componentArrays[physics_2d_handle];
    for(unsigned int i = 0; i < ecdb->_maxEntities; ++i)
    {
